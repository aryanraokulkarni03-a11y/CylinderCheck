...
## 🛠️ Tech Stack
- **Frontend**: React (Vite) + Vanilla CSS (Premium Micro-interactions)
- **Database**: Supabase (PostgreSQL)
- **APIs**: India Post PIN API for real-time location validation
- **Hosting**: Vercel

## 💰 Roadmap & Monetization
- **Phase 1**: Crowdsourced wait-time heatmaps for every major Indian city.
- **Phase 2**: Premium "Plus" alerts via WhatsApp and SMS for high-demand zones.
- **Phase 3**: Commercial dashboard for businesses (restaurants/hotels) to manage high-volume logistics.

## 🚀 Getting Started
1. **Clone & Install**: `git clone` followed by `npm install`.
2. **Environment**: Copy `.env.example` to `.env.local` and add your Supabase credentials.
