...
CREATE INDEX IF NOT EXISTS alert_dispatch_jobs_status_idx
  ON alert_dispatch_jobs (status, created_at DESC);

CREATE INDEX IF NOT EXISTS alert_dispatch_jobs_reminder_idx
  ON alert_dispatch_jobs (reminder_type, delivery_channel, created_at DESC);

CREATE INDEX IF NOT EXISTS alert_dispatch_jobs_provider_idx
  ON alert_dispatch_jobs (provider, created_at DESC);

CREATE OR REPLACE FUNCTION touch_alert_dispatch_job_updated_at()
RETURNS TRIGGER
LANGUAGE plpgsql
...
DROP TRIGGER IF EXISTS alert_dispatch_jobs_set_updated_at
  ON alert_dispatch_jobs;

CREATE TRIGGER alert_dispatch_jobs_set_updated_at
  BEFORE UPDATE ON alert_dispatch_jobs
  FOR EACH ROW
  EXECUTE FUNCTION touch_alert_dispatch_job_updated_at();

ALTER TABLE alert_dispatch_jobs ENABLE ROW LEVEL SECURITY;

CREATE TABLE IF NOT EXISTS alert_dispatch_attempts (
  id                  BIGSERIAL PRIMARY KEY,
  job_id              BIGINT NOT NULL REFERENCES alert_dispatch_jobs(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS alert_dispatch_attempts_job_idx
  ON alert_dispatch_attempts (job_id, created_at DESC);

CREATE INDEX IF NOT EXISTS alert_dispatch_attempts_subscription_idx
  ON alert_dispatch_attempts (subscription_id, created_at DESC);

CREATE INDEX IF NOT EXISTS alert_dispatch_attempts_status_idx
  ON alert_dispatch_attempts (status, created_at DESC);

CREATE UNIQUE INDEX IF NOT EXISTS alert_dispatch_attempts_job_subscription_attempt_idx
  ON alert_dispatch_attempts (job_id, subscription_id, attempt_number);

CREATE OR REPLACE FUNCTION touch_alert_dispatch_attempt_updated_at()
RETURNS TRIGGER
LANGUAGE plpgsql
SET search_path = public, pg_temp
AS $$
...
DROP TRIGGER IF EXISTS alert_dispatch_attempts_set_updated_at
  ON alert_dispatch_attempts;

CREATE TRIGGER alert_dispatch_attempts_set_updated_at
  BEFORE UPDATE ON alert_dispatch_attempts
  FOR EACH ROW
  EXECUTE FUNCTION touch_alert_dispatch_attempt_updated_at();

ALTER TABLE alert_dispatch_attempts ENABLE ROW LEVEL SECURITY;

CREATE OR REPLACE VIEW alert_dispatch_health_v1
...
REVOKE ALL ON TABLE alert_dispatch_jobs FROM public, anon, authenticated;
REVOKE ALL ON TABLE alert_dispatch_attempts FROM public, anon, authenticated;
REVOKE ALL ON TABLE alert_dispatch_health_v1 FROM public, anon, authenticated;
...
CREATE TABLE IF NOT EXISTS news_article_candidates (
  id                        BIGSERIAL PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS news_article_candidates_review_idx
  ON news_article_candidates (review_status, publish_status, created_at DESC);

CREATE INDEX IF NOT EXISTS news_article_candidates_source_idx
  ON news_article_candidates (source_key, published_source_at DESC);

CREATE INDEX IF NOT EXISTS news_article_candidates_city_idx
  ON news_article_candidates (city, category, published_source_at DESC);

CREATE INDEX IF NOT EXISTS news_article_candidates_slug_idx
  ON news_article_candidates (slug);

ALTER TABLE news_article_candidates ENABLE ROW LEVEL SECURITY;

CREATE TABLE IF NOT EXISTS news_article_publications (
  id                    BIGSERIAL PRIMARY KEY,
  candidate_id          BIGINT REFERENCES news_article_candidates(id) ON DELETE SET NULL,
...
CREATE INDEX IF NOT EXISTS news_article_publications_published_idx
  ON news_article_publications (publish_status, published_at DESC);
...
CREATE POLICY "Anyone can read published news articles"
  ON news_article_publications
  FOR SELECT
  USING (publish_status = 'published');

CREATE OR REPLACE FUNCTION touch_news_editorial_updated_at()
RETURNS TRIGGER
LANGUAGE plpgsql
SET search_path = public, pg_temp
AS $$
...
DROP TRIGGER IF EXISTS news_article_candidates_set_updated_at
  ON news_article_candidates;

CREATE TRIGGER news_article_candidates_set_updated_at
  BEFORE UPDATE ON news_article_candidates
  FOR EACH ROW
  EXECUTE FUNCTION touch_news_editorial_updated_at();

DROP TRIGGER IF EXISTS news_article_publications_set_updated_at
  ON news_article_publications;

CREATE TRIGGER news_article_publications_set_updated_at
  BEFORE UPDATE ON news_article_publications
  FOR EACH ROW
  EXECUTE FUNCTION touch_news_editorial_updated_at();

CREATE OR REPLACE VIEW news_review_queue_v1
WITH (security_invoker = true) AS
SELECT
  id,
  candidate_key,
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'news_article_candidates_source_key_fkey'
  ) THEN
    ALTER TABLE news_article_candidates
      ADD CONSTRAINT news_article_candidates_source_key_fkey
      FOREIGN KEY (source_key)
      REFERENCES scrape_source_registry(source_key)
      ON DELETE RESTRICT;
  END IF;
END;
$$;
